...
TEST_DATA = "/data/kaggle/test" 
ADDITIONAL_DATA = "/data/kaggle/additional"
SEG_DATA = 'train_crp'

# types = ['Type_1']#,'Type_2','Type_3']
# type_ids = []

for dirs in ['train_crp', 'train_crp/Type_1', 'train_crp/Type_2', 'train_crp/Type_3']:
    if not os.path.exists(dirs):
        os.makedirs(dirs)

def get_img_ids(fold):
    type_i_files = glob(os.path.join(TRAIN_DATA, fold, "*.jpg"))
    type_i_ids = np.array([s[len(TRAIN_DATA)+8:-4] for s in type_i_files])
    # type_ids.append(type_i_ids[:5])
    return type_i_ids

def check_ids(fold):
    type_i_files = glob(os.path.join(SEG_DATA, fold, "*.jpg"))
    type_i_ids = np.array([s[len(SEG_DATA)+8:-4] for s in type_i_files])
    return type_i_ids
...
def show_rectangle(image_id, image_type, ret):
    img = get_image_data(image_id, image_type)
    if(img.shape[0] > img.shape[1]):
                            tile_size = (int(img.shape[1]*256/img.shape[0]), 256)
    else:
                            tile_size = (256, int(img.shape[0]*256/img.shape[1]))
    img = cv2.resize(img, dsize=tile_size)
    img = cv2.rectangle(img, (ret[0], ret[1]), (ret[0]+ret[2], ret[1]+ret[3]), (255,0,0), 2)
    return img

def crop_img(image_id, image_type, ret):
    img = get_image_data(image_id, image_type)
    if(img.shape[0] > img.shape[1]):
                            tile_size = (int(img.shape[1]*256/img.shape[0]), 256)
    else:
                            tile_size = (256, int(img.shape[0]*256/img.shape[1]))
    img = cv2.resize(img, dsize=tile_size)
    img = img[ret[1]:(ret[1]+ret[3]), ret[0]:(ret[0]+ret[2])]
    return img 

def crop_save(image_id, image_type):
    img_id, img, ret = get_and_crop_image(image_id, image_type)
    img = crop_img(img_id, image_type, ret)
    img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
    fn = os.path.join('train_crp', image_type, img_id + '.jpg') 
    cv2.imwrite(fn, img)
    return

# import time
# print(time.strftime("%c"))
for img_type in ['Type_1','Type_2','Type_3']:
    img_ids = get_img_ids(img_type)
    ext_ids = check_ids(img_type)
    # broken files
    ext_ids = np.append(ext_ids, '1339')
    img_ids = np.setdiff1d(img_ids, ext_ids)
    print("crop for " + img_type)
    # only 50 files
    for iid in img_ids[:50]:
    # all files
    # for iid in img_ids:
        crop_save(iid, img_type)
